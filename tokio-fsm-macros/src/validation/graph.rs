...
use super::{
    types::FsmStructure,
    validator::{GraphDiagnostic, validate_graph},
};
...
impl FsmStructure {
    ...
    pub fn validate(&self) -> syn::Result<()> {
        ...
        let diagnostics = validate_graph(&self.initial_state.to_string(), &states, &edges);
    ...
    fn to_edges(&self, state_spans: &HashMap<String, Ident>) -> syn::Result<Vec<(String, String)>> {
    ...
    fn duplicate_handlers_for(&self, state: &Ident, event: &Ident) -> Option<(&Ident, &Ident)> {
        let mut first = None;
        ...
        for handler in &self.handlers {
            ...
            if !handler.source_states.iter().any(|source| source == state) {
                continue;
            }
...
mod tests {
    ...
    fn test_valid_fsm_passes_validation() {
        ...
        let input: syn::ItemImpl = parse_str(
            r#"
            impl MyFsm {
                type Context = ();
                type Error = ();
